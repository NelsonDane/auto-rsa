...
from tastytrade.order import (Order, OrderDetails, OrderPriceEffect, OrderType,
                              TimeInForce)
...
async def day_trade_check(session, acct, cash_balance):
    if acct.is_margin and cash_balance <= 25000:
        ...
        async with aiohttp.request(
                'GET', url, headers=session.get_request_headers()) as response:
            if response.status != 200:
                raise Exception(
                    'Could not get trading accounts trading-status from Tastyworks...'
                )
            data = (await response.json())['data']
            print(
                f"Tastytrade account {acct.account_number}: day trade count is {int(data['day-trade-count'])}."
            )
...
def order_setup(order_type, stock_price, stock, amount):
    if order_type == ['Market', 'Debit', 'Buy to Open']:
        action = 'Buy to Open'
        details = OrderDetails(type=OrderType.MARKET,
    ...
    leg = Equity(action=action, ticker=stock, quantity=amount)
...
def tastytrade_init():
    try:
        ...
        if not os.getenv("TASTYTRADE_USERNAME") or not os.getenv(
                "TASTYTRADE_PASSWORD"):
    ...
    for acct in accounts:
        ...
        if len(positions) == 1:
            ...
            current_price.append(
                positions[0]['average-daily-market-close-price'])
        else:
        ...
        for value in stocks:
            ...
            temp_value = round((float(amounts[i]) * float(current_price[i])),
                               2)
            current_value.append(temp_value)
        print(f"Holdings on Tastytrade account {acct.account_number}")
        if ctx:
            await ctx.send(
                f"Holdings on Tastytrade account {acct.account_number}")
...
async def tastytrade_transaction(tt,
                                 action,
                                 stock,
                                 amount,
                                 price,
    ...
    for index, acct in enumerate(accounts):
        if not DRY:
            ...
            if day_trade_ok:
                stock_limit = await streamer.stream(EventType.PROFILE,
                                                    stock_list)
                stock_quote = await streamer.stream(EventType.QUOTE,
                                                    stock_list)
                if all_amount:
                ...
                if action == 'buy':
                    ...
                    new_order = order_setup(order_type, stock_price, stock,
                                            amount)
                elif action == 'sell':
                    ...
                    new_order = order_setup(order_type, stock_price, stock,
                                            amount)
                try:
                    json_response = await accounts[index].execute_order(
                        new_order, tt, dry_run=DRY)
                    if json_response != {}:
                        if json_response['order']['status'] == 'Routed':
                            print(
                                f"Tastytrade account {acct.account_number}: {action} {amount} of {stock}"
                            )
                            if ctx:
                                await ctx.send(
                                    f"Tastytrade account {acct.account_number}: {action} {amount} of {stock}"
                            ...
                            print(
                                f"Tastytrade account {acct.account_number} Error: {error}"
                            )
                            if ctx:
                                await ctx.send(
                                    f"Tastytrade account {acct.account_number} Error: {error}"
                    ...
                    if error[0] == 'Unknown remote error 422' or error[
                            0] in 'Order execution failed ':
                        print(
                            "Tastytrade: Error placing MARKET order trying LIMIT..."
                        )
                        if ctx:
                            await ctx.send(
                                "Tastytrade: Error placing MARKET order trying LIMIT..."
                            )
                        stock_limit = await streamer.stream(
                            EventType.PROFILE, stock_list)
                        stock_quote = await streamer.stream(
                        ...
                        if action == 'buy':
                            ...
                            order_type = ['Market', 'Debit', 'Buy to Open']
                            new_order = order_setup(order_type, stock_price,
                                                    stock, amount)
                        elif action == 'sell':
                            stock_limit = D(stock_limit[0].lowLimitPrice)
                            if stock_limit.is_nan():
                            ...
                            new_order = order_setup(order_type, stock_price,
                                                    stock, amount)
                        try:
                            json_response = await accounts[
                                index].execute_order(new_order,
                                                     tt,
                                                     dry_run=DRY)
                        except Exception as error_json:
                            error_json = str(error_json)
                        ...
                        error = error_json['error']['errors'][0]['message']
                        print(
                            f"Tastytrade: Error occured placing order... {error}"
                        )
                        if ctx:
                            await ctx.send(
                                f"Tastytrade: Error occured placing order... {error}"
                ...
                print(
                    f"Tastytrade account {acct.account_number}: day trade count is >= 3 skipping..."
                )
                print(
            ...
            print(
                f"Tastytrade: Running in DRY mode. Transaction would've been: {action} {amount} of {stock}"
            )
            if ctx:
                await ctx.send(
                    f"Tastytrade: Running in DRY mode. Transaction would've been: {action} {amount} of {stock}"
