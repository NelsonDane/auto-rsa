...
def fidelity_init(DOCKER=False):
    try:
        ...
        if not os.getenv("FIDELITY_USERNAME") or not os.getenv(
                "FIDELITY_PASSWORD"):
        ...
        username_field = driver.find_element(by=By.CSS_SELECTOR,
                                             value="#userId-input")
        username_field.send_keys(FIDELITY_USERNAME)
        WebDriverWait(driver, 10).until(
            expected_conditions.element_to_be_clickable(
                (By.CSS_SELECTOR, "#password")))
        password_field = driver.find_element(by=By.CSS_SELECTOR,
                                             value="#password")
        password_field.send_keys(FIDELITY_PASSWORD)
        driver.find_element(by=By.CSS_SELECTOR,
                            value="#fs-login-button").click()
    ...
    try:
        ...
        if ctx and loop:
            asyncio.ensure_future(ctx.send(
                f'Total Fidelity account value: {total_value[0].text}'),
                                  loop=loop)
        # Get value of individual and retirement accounts
        ind_accounts = driver.find_elements(by=By.CSS_SELECTOR,
                                            value='[data-group-id="IA"]')
        ret_accounts = driver.find_elements(by=By.CSS_SELECTOR,
                                            value='[data-group-id="RA"]')
        ...
        if ret_acc:
            ...
            if ctx and loop:
                asyncio.ensure_future(ctx.send("Retirement accounts:"),
                                      loop=loop)
            for x in range(len(ret_num)):
                print(f'{ret_num[x]} value: {ret_val[x]}')
                if ctx and loop:
                    asyncio.ensure_future(
                        ctx.send(f'{ret_num[x]} value: {ret_val[x]}'),
                        loop=loop)
...
def fidelity_transaction(driver,
                         action,
                         stock,
                         amount,
                         price,
    ...
    try:
        ...
        test = driver.find_element(by=By.CSS_SELECTOR,
                                   value="#ett-acct-sel-list")
    ...
    for x in range(number_of_accounts):
        try:
            ...
            driver.execute_script("arguments[0].click();",
                                  accounts_dropdown_in)
            WebDriverWait(driver, 10).until(
                expected_conditions.presence_of_element_located(
                    (By.ID, "ett-acct-sel-list")))
            test = driver.find_element(by=By.ID, value="ett-acct-sel-list")
            accounts_dropdown_in = test.find_elements(by=By.CSS_SELECTOR,
                                                      value="li")
            account_label = accounts_dropdown_in[x].text
            accounts_dropdown_in[x].click()
            sleep(1)
            # Type in ticker
            ticker_box = driver.find_element(by=By.CSS_SELECTOR,
                                             value="#eq-ticket-dest-symbol")
            WebDriverWait(driver, 10).until(
            ...
            try:
                ...
                pass
            # Get ask/bid price
            ask_price = (driver.find_element(
                by=By.CSS_SELECTOR,
                value=
                "#quote-panel > div > div.eq-ticket__quote--blocks-container > div:nth-child(2) > div > span > span"
            )).text
            bid_price = (driver.find_element(
                by=By.CSS_SELECTOR,
                value=
                "#quote-panel > div > div.eq-ticket__quote--blocks-container > div:nth-child(1) > div > span > span"
            )).text
            ...
            amount_box = driver.find_element(by=By.CSS_SELECTOR,
                                             value="#eqt-shared-quantity")
            amount_box.clear()
            ...
            WebDriverWait(driver, 10).until(check_if_page_loaded)
            sleep(1)
            preview_button = driver.find_element(by=By.CSS_SELECTOR,
                                                 value="#previewOrderBtn")
            ...
            if not DRY:
                ...
                try:
                    place_button = driver.find_element(by=By.CSS_SELECTOR,
                                                       value="#placeOrderBtn")
                    place_button.click()
