...
def fidelity_init(DOCKER=False):
    try:
        ...
        password_field = driver.find_element(
            by=By.CSS_SELECTOR, value="#password")
    ...
    try:
        ...
        if ctx and loop:
            asyncio.ensure_future(
                ctx.send(f'Total Fidelity account value: {total_value[0].text}'), loop=loop)
        ...
        if ret_acc:
            ...
            if ctx and loop:
                asyncio.ensure_future(
                    ctx.send("Retirement accounts:"), loop=loop)
    ...
    for x in range(number_of_accounts):
        try:
            ...
            driver.execute_script(
                "arguments[0].click();", accounts_dropdown_in)
            ...
            ask_price = (driver.find_element(
                by=By.CSS_SELECTOR, value="#quote-panel > div > div.eq-ticket__quote--blocks-container > div:nth-child(2) > div > span > span")).text
            bid_price = (driver.find_element(
                by=By.CSS_SELECTOR, value="#quote-panel > div > div.eq-ticket__quote--blocks-container > div:nth-child(1) > div > span > span")).text
            ...
            if action == "buy":
                buy_button = driver.find_element(
                    by=By.CSS_SELECTOR, value="#action-buy > s-root > div > label > s-slot > s-assigned-wrapper")
                buy_button.click()
            elif action == "sell":
                sell_button = driver.find_element(
                    by=By.CSS_SELECTOR, value="#action-sell > s-root > div > label > s-slot > s-assigned-wrapper")
                sell_button.click()
            else:
            ...
            if not LIMIT:
                market_button = driver.find_element(
                    by=By.CSS_SELECTOR, value="#market-yes > s-root > div > label > s-slot > s-assigned-wrapper")
                market_button.click()
            else:
                limit_button = driver.find_element(
                    by=By.CSS_SELECTOR, value="#market-no > s-root > div > label > s-slot > s-assigned-wrapper")
                limit_button.click()
                # Set price
                if action == "buy":
            ...
            if not DRY:
                ...
                try:
                    ...
                    WebDriverWait(driver, 10).until(
                        expected_conditions.presence_of_element_located(
                            (By.XPATH, "(//button[@class='pvd-modal__close-button'])[3]"))
                    )
                    error_dismiss = driver.find_element(
                        by=By.XPATH, value="(//button[@class='pvd-modal__close-button'])[3]")
                    driver.execute_script(
