        ...
        if positions == []:
            ...
            if ctx and loop:
                asyncio.ensure_future(
                    ctx.send("No holdings in Robinhood"), loop=loop)
        else:
            print("Holdings in Robinhood:")
            if ctx and loop:
                asyncio.ensure_future(
                    ctx.send("Holdings in Robinhood:"), loop=loop)
            for item in positions:
                ...
                if ctx and loop:
                    asyncio.ensure_future(
                        ctx.send(f"{sym}: {qty} @ ${(current_price)} = ${total_value}"), loop=loop)
    except Exception as e:
    ...
    if not DRY:
        ...
        if ctx and loop:
            asyncio.ensure_future(ctx.send(
                f"Robinhood: Running in DRY mode. Trasaction would've been: {action} {amount} of {stock}"), loop=loop)
