...
def robinhood_holdings(rh, ctx=None, loop=None):
    ...
    try:
        ...
        if positions == []:
            ...
            if ctx and loop:
                asyncio.ensure_future(ctx.send("No holdings in Robinhood"),
                                      loop=loop)
        else:
            print("Holdings in Robinhood:")
            if ctx and loop:
                asyncio.ensure_future(ctx.send("Holdings in Robinhood:"),
                                      loop=loop)
        ...
        if ctx and loop:
            asyncio.ensure_future(
                ctx.send(f'Robinhood: Error getting account holdings: {e}'),
                loop=loop)


def robinhood_transaction(rh,
                          action,
                          stock,
                          amount,
                          price,
    ...
    if not DRY:
        try:
            ...
            if action == "buy":
                ...
                if ctx and loop:
                    asyncio.ensure_future(
                        ctx.send(f"Robinhood: Bought {amount} of {stock}"),
                        loop=loop)
            # Sell Market order
            elif action == "sell":
                if all_amount:
                ...
                if ctx and loop:
                    asyncio.ensure_future(
                        ctx.send(f"Robinhood: Sold {amount} of {stock}"),
                        loop=loop)
            else:
            ...
            if ctx and loop:
                asyncio.ensure_future(
                    ctx.send(f'Robinhood: Error submitting order: {e}'),
                    loop=loop)
    else:
        print(
