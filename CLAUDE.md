...
**Dispatch API (`src/dispatch_api.py`) — a third transport into `arg_parser` → `fun_run`.** When `DISPATCH_API_KEY` is set, `main()` runs a FastAPI app (`create_app()`) alongside the Discord bot in the same process via `asyncio.gather`-style concurrency (`_run_bot_and_dispatch_api()`), instead of the old blocking `bot.run(...)`. It exists so other containers — a manual-command bot, a self-bot watching a channel for signals, an SEC-filing scraper sourcing reverse-split candidates — can submit orders without holding any brokerage credentials themselves; this container remains the only one with sessions/cookies. `POST /command` (auth'd via an `X-API-Key` header, compared with `secrets.compare_digest`) takes the exact same token list `arg_parser` already accepts from the CLI and `!rsa` (e.g. `{"args": ["buy", "1", "AAPL", "robinhood", "true"]}`) — no separate schema to keep in sync. Results are reported through the same `bot`/`loop` already running here via `print_and_discord`, so the Discord channel stays the one place a human sees order outcomes or an OTP/captcha prompt, regardless of which producer container originated the request. A single `order_lock` (`asyncio.Lock`) serializes *every* `fun_run` call — both from `!rsa` and from `/command` — because two concurrent calls against a browser-automation broker (e.g. Wells Fargo) would race the same on-disk session/cookie file. `GET /health` is unauthenticated for container healthchecks. The API is bound to `0.0.0.0` but `docker-compose.yml` only `expose`s it on the compose network (not `ports:` to the host) — other bot containers must join the same named `auto-rsa-dispatch` network and reach it at `http://auto-rsa:8000`; there's no host-level exposure to firewall around. Embedding uvicorn inside the same loop as discord.py needs two non-obvious things: a `uvicorn.Server` subclass overriding `install_signal_handlers()` as a no-op (discord.py already owns SIGINT/SIGTERM on this loop — installing uvicorn's own too means they fight over the same signals), and `server.should_exit = True` (not `server.shutdown()`, which does nothing useful while `main_loop()` is still running) to stop it gracefully.
