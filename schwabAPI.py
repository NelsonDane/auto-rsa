...
def schwab_init():
    ...
    try:
        ...
        schwab.login(username=SCHWAB_USERNAME,
                     password=SCHWAB_PASSWORD, totp_secret=SCHWAB_TOTP_SECRET)
...
def schwab_transaction(schwab, action, stock, amount, price, time, DRY=True, ctx=None, loop=None):
    ...
    for account in list(schwab.get_account_info().keys()):
        ...
        if DRY:
            ...
            if ctx and loop:
                asyncio.ensure_future(
                    ctx.send(f"Running in DRY mode. No transactions will be made."), loop=loop)
        try:
            ...
            if ctx and loop:
                asyncio.ensure_future(ctx.send(
                    f"Schwab account {account}: The order verification was " + "successful" if success else "unsuccessful"), loop=loop)
                if not success:
                    asyncio.ensure_future(ctx.send(
                        f"Schwab account {account}: The order verification produced the following messages: "), loop=loop)
                    asyncio.ensure_future(ctx.send(f"{messages}"), loop=loop)
        except Exception as e:
            print(f'Schwab {account}: Error submitting order: {e}')
            if ctx and loop:
                asyncio.ensure_future(
                    ctx.send(f'Schwab {account}: Error submitting order: {e}'), loop=loop)
            return None
        sleep(1)
        print()
