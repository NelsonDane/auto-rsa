...
def ally_init(ALLY_EXTERNAL=None, DOCKER=False, botObj=None, loop=None):
    ...
    for account in accounts:
        ...
        try:
            ...
            # TODO: Is there a way to specify the website size instead of the window size in Selenium?
            ...
            # TODO: I have a lot of these sleep calls because I am not sure how to wait for the page content
            #  to be loaded in an intelligent way. This needs changed.
            sleep(5)
            WebDriverWait(driver, 10).until(check_if_page_loaded)

            sleep(3)
            if "send-code" in driver.current_url:
                ...
                if botObj is not None and loop is not None:
                    ...
                    if "register-device" in driver.current_url.lower():
                        ...
                        sleep(5)
                    # TODO It is not actually called error page. Get the 20 minute time out and figure the real name.
                    elif "errorpage" in driver.current_url.lower():
                        raise Exception(
...
def ally_transaction(ally_o: Brokerage, orderObj: stockOrder, loop=None):
    ...
    for s in orderObj.get_stocks():
        for key in ally_o.get_account_numbers():
            ...
            # Go to trade page
            driver.get(
                "https://digital.ally.com/ftgw/digital/trade-equity/index/orderEntry"
            )
            ...
            try:
                ...
                ally_error(driver, f"No accounts found in dropdown: {e}")
                killSeleniumDriver(ally_o)
                return None
            ...
            for x in range(number_of_accounts):
                try:
                    ...
                    account_number = ally_o.get_account_numbers(key)[x]
