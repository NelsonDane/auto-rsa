    ...
    if accountName in supported_brokerages:
        ...
        try:
            if accountName == "fidelity" or accountName == "all":
                await fidelity_holdings(fidelity_account if AO is None else AO,
                                        ctx)
        except:
            pass
        try:
        ...
        try:
            if accountName == "tasty" or accountName == "tastytrade" or accountName == "all":
                await tastytrade_holdings(tastytrade if AO is None else AO,
                                          ctx)
        except:
            pass
    else:
...
async def place_order(wanted_action,
                      wanted_amount,
                      wanted_stock,
                      single_broker,
                      AO=None,
    ...
    try:
        ...
        print("==========================================================")
        print(
            f"Order: {wanted_action} {wanted_amount} {grammar} of {wanted_stock} on {single_broker}"
        )
        print("==========================================================")
        print()
        ...
        if single_broker == "all":
            # Ally
            await ally_transaction(ally_account if AO is None else AO,
                                   wanted_action, wanted_stock, wanted_amount,
                                   wanted_price, wanted_time, DRY, ctx)
            # Fidelity
            await fidelity_transaction(fidelity_account if AO is None else AO,
                                       wanted_action, wanted_stock,
                                       wanted_amount, wanted_price,
                                       wanted_time, DRY, ctx)
            # Robinhood
            await robinhood_transaction(robinhood if AO is None else AO,
                                        wanted_action, wanted_stock,
                                        wanted_amount, wanted_price,
                                        wanted_time, DRY, ctx)
            # Schwab
            await schwab_transaction(schwab if AO is None else AO,
                                     wanted_action, wanted_stock,
                                     wanted_amount, wanted_price, wanted_time,
                                     DRY, ctx)
            # Webull
            # await webull_transaction(webull_account, wanted_action, wanted_stock, wanted_amount, wanted_price, wanted_time, DRY, ctx)
            # Tradier
            await tradier_transaction(tradier if AO is None else AO,
                                      wanted_action, wanted_stock,
                                      wanted_amount, wanted_price, wanted_time,
                                      DRY, ctx)
            # Tastytrade
            await tastytrade_transaction(tastytrade if AO is None else AO,
                                         wanted_action, wanted_stock,
                                         wanted_amount, wanted_price,
                                         wanted_time, DRY, ctx)
...
if __name__ == "__main__":
    ...
    async def rsa(ctx, wanted_action, wanted_amount, wanted_stock,
                  wanted_account, DRY):
        # Convert string to boolean
        DRY = await stringToBool(DRY)
        print(DRY)
        try:
            await place_order(wanted_action=wanted_action,
                              wanted_amount=wanted_amount,
                              wanted_stock=wanted_stock,
                              single_broker=wanted_account,
                              DRY=DRY,
                              ctx=ctx)
