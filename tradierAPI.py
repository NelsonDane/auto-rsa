            ...
            for position in stocks:
                ...
                if ctx and loop:
                    asyncio.ensure_future(ctx.send(
                        f"{position}: {amounts[i]} @ ${current_price[i]} = ${current_value[i]}"), loop=loop)
        except Exception as e:
            print(f"Tradier {account_number}: Error getting holdings: {e}")
            if ctx and loop:
                asyncio.ensure_future(
                    ctx.send(f"Tradier {account_number}: Error getting holdings: {e}"), loop=loop)
    ...
    for account_number in tradier:
        if not DRY:
            response = requests.post(f'https://api.tradier.com/v1/accounts/{account_number}/orders',
                                     data={'class': 'equity', 'symbol': stock, 'side': action,
                                           'quantity': amount, 'type': 'market', 'duration': 'day'},
                                     headers={
            ...
            try:
                if json_response['order']['status'] == "ok":
                    ...
                    if ctx and loop:
                        asyncio.ensure_future(ctx.send(
                            f"Tradier account {account_number}: {action} {amount} of {stock}"), loop=loop)
                else:
                    print(
                        f"Tradier account {account_number} Error: {json_response['order']['status']}")
                    if ctx and loop:
                        asyncio.ensure_future(ctx.send(
                            f"Tradier account {account_number} Error: {json_response['order']['status']}"), loop=loop)
                    return None
            except KeyError:
                print(
                ...
                if ctx and loop:
                    asyncio.ensure_future(ctx.send(
                        f"Tradier account {account_number} Error: This order did not route. Is this a new account?"), loop=loop)
        else:
            print(
                f"Tradier account {account_number}: Running in DRY mode. Trasaction would've been: {action} {amount} of {stock}")
            if ctx and loop:
                asyncio.ensure_future(ctx.send(
                    f"Tradier account {account_number}: Running in DRY mode. Trasaction would've been: {action} {amount} of {stock}"), loop=loop)
